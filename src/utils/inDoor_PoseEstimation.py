...
def inDoor_PoseEstimation_F(assets_dir):
    ...
    for filename in os.listdir(input_dir):
        ...
        frame1 = np.zeros((frameHeight, frameWidth, 3), dtype=np.uint8)
        ...
        for i in range(nPoints):
            ...
            if prob > threshold:
                cv2.circle(frameCopy, (int(x), int(y)), 20, (0, 0, 255), thickness=-1, lineType=cv2.FILLED)
        ...
        for pair in POSE_PAIRS:
            ...
            if points[partA] and points[partB]:
                cv2.line(frame1, points[partA], points[partB], (255, 255, 255), 40)
                cv2.circle(frame1, points[partA], 20, (0, 0, 255), thickness=-1, lineType=cv2.FILLED)

        # Resize the output image
        output_resized = cv2.resize(frame1, (500, 500))
